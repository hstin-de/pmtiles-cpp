    ...
    bool compress /* gzip if true */)
{
    ...
    for (const auto &e : entries)
        writeUVarint(raw, e.RunLength);
    for (const auto &e : entries)
        writeUVarint(raw, e.Length);
...
static std::string buildMetadataJSON(uint8_t minZoom, uint8_t maxZoom)
{
    ...
    s += "\"format\":\"webp\",";
    s += "\"minzoom\":";
    s += std::to_string((int)minZoom);
    s += ",";
    s += "\"maxzoom\":";
    s += std::to_string((int)maxZoom);
    s += ",";
    s += "\"name\":\"Map Generated at ";
    s += when;
    s += "\",";
    s += "\"type\":\"overlay\",";
    s += "\"version\":\"1.0\",";
    s += "\"description\":\"Map Generated at ";
    s += when;
    s += "\"";
    s += "}";
    return s;
                                                   ...
                                                   uint8_t **out_buf, int *out_len)
{
    ...
    std::vector<pmt::EntryV3> entries;
    std::vector<UniqueRef> uniques;
    if (!prepare_entries(tiles, count, minZoom, maxZoom, entries, uniques))
        return 0;
